...
\section{Model B (add $J$ retrospective products via discrepancy states)}
...
\subsection{Full conditional of $v_t$ (GIG)}
...
\subsection{Full conditional of $s_t$ (truncated Normal)}
...
\subsection{Full conditional of $\sigma$ (GIG)}
...
\subsection{Full conditional of $\gamma$ (non-conjugate; target density)}
...
\subsection{Joint posterior kernel for $(\sigma,\gamma)$ for one source}
...
\subsection{Evolution covariance $W$ (inverse-Wishart conjugacy)}
...
\quad
V_{s,n}=\left(1+\frac{(d^s)^2}{R_n}\right)^{-1}
=\left(1+\frac{(C^s)^2\,\sigma^s(\gamma^s)^2}{B^s\,v_n}\right)^{-1},
\quad
m_{s,n}=V_{s,n}\left(\frac{d^s\,y_n^\circ}{R_n}\right)
...
\begin{equation}
\bm x_{0:\text{end}} \mid \text{rest} \;\;\text{is Gaussian and sampled by FFBS for the DLM}\;\;
\tilde{\bm y}_\tau \mid \bm x_\tau \sim \N(\bm H_\tau^\T\bm x_\tau,\bm R_\tau),
\qquad
...
(These follow from $\tilde y_n/R_n=(y_n-\eta_n)/(\sigma^sB^s v_n)-(C^s|\gamma^s|/B^s)\,s_n/v_n-A^s/(\sigma^sB^s)$.)
...
\noindent Also, consider s := |\mathcal{I}_s|.
...
% ===========================
\subsection{State update (trans-dimensional Gaussian block): FFBS for MCMC and FBBS/RTS for VB}
...
\subsubsection*{VB: Gaussian variational smoother (FBBS/RTS) for $q(\bm x_{0:\mathcal{T}})$}
...
% NEW SECTION TO INSERT
% ===========================
\subsection{Laplace--Delta approximation for $q(\sigma^s,\gamma^s)$ and ELBO contributions}
...
Many CAVI updates require expectations of the form $\E_q[g(\sigma^s,\gamma^s)]$. Under \eqref{eq:q_u_xi_and_induced},
for any twice-differentiable $g(u,\xi)$ define $g^\sharp(\sigma,\gamma):=g(\log\sigma,\xi(\gamma))$.
With $Z=(u^s,\xi^s)\sim \N(\hat\mu_s,\hat\Sigma_s)$, the (second-order) Delta method gives
\begin{equation}
\E_q[g^\sharp(\sigma^s,\gamma^s)]
=\E_{q_s(u,\xi)}[g(u,\xi)]
...
\subsection{State-block contribution $\mathcal{L}_{x}(q)$}
...
\subsection{Inverse-Wishart block contributions $\mathcal{L}_{W_b}(q)$}
...
\subsection{Latent truncation prior for $s_n$: $\mathcal{L}_{n}^{(s\text{-prior})}(q)$}
...
\subsection{Entropy of $q(v_n)$ (GIG) and $q(s_n)$ (truncated Normal)}
...
\subsection{Nonconjugate $(\sigma^s,\gamma^s)$ contribution}

...
\newcommand{\phiN}[1]{\varphi\!\left(#1\right)}
...
Scope: this document covers the augmented exAL derivations for Models A--C only; it excludes non-exAL likelihood variants and non-augmented inference schemes.
...
\section{Model C-T (forecast period: ensemble forecasts; transfer retained)}
\label{sec:model_c_transfer_retained}

This alternative forecast-stage specification keeps the transfer block active during
$t=T+1,\dots,T+K_{(1)}$.
Let $r:=1+m$ denote the transfer-state dimension (for $(\zeta_t,\bm\psi_t)$).

\paragraph{Segment-wise state (active forecasters may shrink by lead time).}
For forecast segment $s$ (with $a_s$ active forecasters), define
\[
\bm\beta_t^{(s)}=
\begin{bmatrix}
\bm\theta_t\\
...
p_t^{(s)}=q(1+a_s)+r.
\]
Compared with Model C, the transfer block $\bm\tau_t$ is \emph{not} dropped.

\paragraph{Forecast observation equation (source/member level).}
For active source $j\in\{1,\dots,a_s\}$ and member $i$:
\begin{align}
y_T^{j,i}(k)\mid \bm\beta_{T+k}^{(s)},\sigma^j,\gamma^j
&\sim \mathrm{exAL}_{p_0}\!\big((\bm h_{T+k,j}^{(s)})^\T\bm\beta_{T+k}^{(s)},\;\sigma^j,\;\gamma^j\big),
\end{align}
where $\bm h_{T+k,j}^{(s)}$ loads (i) baseline state $\bm\theta_t$, (ii) discrepancy block $\bm\delta_t^j$,
and (iii) transfer contribution via $\zeta_t$ (and any retained transfer regressors).

\paragraph{Forecast transition equation.}
\begin{align}
\bm\beta_t^{(s)}\mid \bm\beta_{t-1}^{(s)}
&\sim \N\!\big(\bm M_t^{(s)}\bm\beta_{t-1}^{(s)},\;\bm W_t^{(s)}\big),\label{eq:CT_state}
\end{align}
with block-diagonal transition
\[
\bm M_t^{(s)}=
\BlockDiag\!\big(
\underbrace{\bm G_t,\dots,\bm G_t}_{1+a_s\ \text{blocks}},
\bm G_t^{\mathrm{trans}}
\big),
\qquad
\bm G_t^{\mathrm{trans}}=
...
\end{bmatrix}.
\]
Thus $\bm x_t$ can continue driving transfer dynamics during forecast.

\paragraph{Boundary map at $T\to T+1$ and across forecast segments.}
Let $\bm x_T^{\mathrm{hist}}\in\R^{q(1+J_f)+r}$ denote the historical terminal state
(including transfer). The first forecast state is obtained by
\[
\bm\beta_{T}^{(1)}=\bm P_{T\to T+1}\bm x_T^{\mathrm{hist}},
\qquad
\bm C_{T}^{(1)}=\bm P_{T\to T+1}\bm C_T^{\mathrm{hist}}\bm P_{T\to T+1}^\T,
\]
where $\bm P_{T\to T+1}$ keeps baseline/discrepancy blocks and the transfer block.
If active forecasters decrease between forecast segments ($a_s\to a_{s+1}$), use
\[
\bm\beta_t^{(s+1)}=\bm P_{s\to s+1}\bm\beta_t^{(s)},
\qquad
\bm C_t^{(s+1)}=\bm P_{s\to s+1}\bm C_t^{(s)}\bm P_{s\to s+1}^\T,
\]
with $\bm P_{s\to s+1}$ dropping only inactive discrepancy blocks while preserving transfer coordinates.

\paragraph{Remark (reduction to Model C).}
Model C is recovered by removing $\bm\tau_t$ from $\bm\beta_t^{(s)}$
and deleting the corresponding rows/columns of $\bm M_t^{(s)}$, $\bm W_t^{(s)}$,
and the observation loadings $\bm h_{t,j}^{(s)}$.
...
\subsection{Computable complete log-joint form}
Define the residual
\[
\varepsilon_t := y_t-\eta_t-C\sigma|\gamma|s_t-Av_t.
\]
Up to additive constants that do not depend on
$(\bm\theta_{0:T},v_{1:T},s_{1:T},\sigma,\gamma)$, the complete augmented log-joint is
\begin{align}
\log p(\bm\theta_{0:T},v_{1:T},s_{1:T},\sigma,\gamma,y_{1:T})
=\;&
\log p(\bm\theta_0)+\log p(\sigma)+\log p(\gamma) \nonumber\\
&-\frac12\sum_{t=1}^T\!\left[
\log|\bm W_t|+
(\bm\theta_t-\bm G_t\bm\theta_{t-1})^\T\bm W_t^{-1}(\bm\theta_t-\bm G_t\bm\theta_{t-1})
\right] \nonumber\\
&-\frac12\sum_{t=1}^T\!\left[
\log(\sigma B v_t)+\frac{\varepsilon_t^2}{\sigma B v_t}
\right]
-T\log\sigma-\sum_{t=1}^T\frac{v_t}{\sigma}
-\frac12\sum_{t=1}^T s_t^2
+\sum_{t=1}^T\log\!\1(v_t>0,s_t>0).
\label{eq:complete_log_joint}
\end{align}
Equation \eqref{eq:complete_log_joint} is the implementation-ready objective used to
derive kernels in Section~7 and expected-block updates in Sections~8--11.
...
\subsection{Monotonicity and audit closure}
For exact CAVI blocks (Gaussian state block, conjugate covariance blocks, and closed-form
$q(v_n),q(s_n)$ updates), coordinate ascent is ELBO non-decreasing by construction.
The nonconjugate block $q(\sigma^s,\gamma^s)$ is updated by Laplace--Delta,
so small non-monotone ELBO fluctuations can occur from approximation error.
In implementation, record the total ELBO and each additive block in
\eqref{eq:full_elbo_decomp}; flag any persistent ELBO decrease as a derivation or numerics issue.

\subsection*{Planned validations}
\begin{itemize}
\item Compare FFBS state moments against direct Gaussian conditioning on toy dimensions.
\item Verify each closed-form conditional kernel by finite-grid normalization checks.
\item Check CAVI fixed-point consistency by re-evaluating all factor parameters from the final moments.
\item Track ELBO components separately and require bounded relative change before stopping.
\end{itemize}

\section*{Audit statement}
This document now contains a complete augmented factorization
\eqref{eq:joint_aug}, an explicit computable log-joint \eqref{eq:complete_log_joint},
distribution-identified full conditionals, VB/CAVI updates with required moments,
and a full ELBO decomposition with block-level computable expressions.
